...
public class TestComputeServer extends ComputeServerTestCase {
	private final ComputeNodeSlot[] nodes = {
			new ComputeNodeSlot("127.0.0.1", Config.COMPUTE_PORT),
			new ComputeNodeSlot("127.0.0.1", Config.COMPUTE_PORT) };
	...
	public void test1Node() throws Exception {
		// operator
		final MySQLExecuteOperator op = new MySQLExecuteOperator(
				new Identifier("1"));
		op.setQueryTracker(this.qTrackerServer.getDescription());
		...
		final ResultSet rs = this
				.executeComputeQuery("SELECT COUNT(*) AS CNT FROM R1;");
	...
	public void test2Nodes() throws Exception {
		...
		// first op
		final MySQLExecuteOperator op1 = new MySQLExecuteOperator(
				new Identifier("1"));
		op1.setQueryTracker(this.qTrackerServer.getDescription());
		...
		// second op
		final MySQLExecuteOperator op2 = new MySQLExecuteOperator(
				new Identifier("2"));
		op2.setQueryTracker(this.qTrackerServer.getDescription());
		...
		final OperatorDesc source = new OperatorDesc(op1.getOperatorId(),
				nodes[0]);
		op2.addSource(source);
		final OperatorDesc consumer = new OperatorDesc(op2.getOperatorId(),
				nodes[1]);
