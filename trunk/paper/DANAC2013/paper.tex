...
%TODO: hard to read. Suggestion: --> Finally, data structures to avoid expensive scans, such as indexes, are missing. There is also a lack efficient implementations for operations such as joins.
...
%TODO: ^^ again, reading flow feels unbalanced (at least for me). --> Compared to existing work that focuses on fixing shortcomings in Hadoop, we take a different approach by designing XDB as middleware for an existing open-source database system to benefit from the maturity of the database.
...
%TODO: I believe, there should be a comma between "supported" and leading, but unsure
%TODO: (proposed partitioning aspects/sort): Also, the shuffle phase in Hadoop is designed to sort all data for partitioning, though this is not generally necessary.
\end{itemize}
...
%TODO: to efficiently store and process
...
%TODO: ^^ these are no opposites; I would enumerate them as, e.g., "First, ..." and "Second, ..."
