...
/**
 * That that combines a SQL Join op and a SQL Unary into one Operator 
 * to avoid Table materialization
 * @author a.c.mueller
 *
 */
public class SQLCombined extends AbstractJoinOperator {
	...
	public void mergeSQLUnaryParent(SQLUnary sqlU){
		...
		rebuildExpression(this.aggExpressions);
		
		rebuildExpression(this.selectExpressions);
		
		rebuildExpression(this.groupExpressions);
		
		rebuildPredicate(this.wherePred);
		
		rebuildPredicate(this.havingPred);
	...
	private void rebuildExpression(Vector<AbstractExpression> absExpressions) {
	...
	private void rebuildPredicate(AbstractPredicate absP){
		if (absP == null) return;
		RenamePredicateCombineVisitor rpCV = new RenamePredicateCombineVisitor(absP);
		rpCV.visit();
	}
